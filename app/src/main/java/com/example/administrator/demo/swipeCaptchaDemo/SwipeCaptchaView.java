package com.example.administrator.demo.swipeCaptchaDemo;
...
//        int gap = mRandom.nextInt(mCaptchaWidth / 2);
        ...
        //随机生成验证码阴影左上角 x y 点，
        int widthPoint = mWidth - mCaptchaWidth - gap;
        Log.e("--mWidth----", mWidth + "-----mWidth-");
        Log.e("--widthPoint----", widthPoint + "-----widthPoint-");
        int heightPoint = mHeight - mCaptchaHeight - gap;
        if (widthPoint < 0) {
        ...
        mCaptchaX = mRandom.nextInt(widthPoint);
        mCaptchaY = mRandom.nextInt(heightPoint);
...
/*        RectF oval = new RectF(mCaptchaX + gap, mCaptchaY - (r), mCaptchaX + gap + r * 2, mCaptchaY + (r));
    ...
    private Bitmap getMaskBitmap(Bitmap mBitmap, Path mask) {
        if (mBitmap != null) {
            ...
            Log.e("mWidth---mWidth---", "mWidth---" + mWidth + "***mHeight---" + mHeight);
